...
A multi-user scale modelling platform reflecting Joe's passion for the hobby. Builders sign in with email magic links, post builds with photos from the community page, and organise their bench (pinned first, then by date), keeping any build private if they want it for themselves only. Members vote for builds they like and the most voted in the last 30 days are featured on the community page. Any member can start a competition with an entry date and a voting date; status advances by date with no admin step. Winners get placement badges and download their 3D-printable trophy STL, which is never in the repo and never public. Each member has a private stash of unbuilt kits shown as model debt, with one kit marked next, a goal of kits to finish by a date, a journal per kit, and optional email reminders. Members find each other on a search page and become friends by request and acceptance; friend lists are private. Members report builds and the admin hides them; photo uploads can be screened by Google SafeSearch when a key is set. No gamification: votes curate and goals are the member's own commitment; there are never points, streaks, badges, or rankings of people.
