...
- `SPRUE_DISCORD_WEBHOOK`: a Discord channel webhook. When set, signed-in members get a feedback page whose messages post to that channel with their name and profile link. Unset, the page and its footer link do not exist. Treat it as a secret.
- `SPRUE_DISCORD_RESULTS`: a Discord channel webhook for competition results. When set, each competition is posted to that channel once when it is decided: the podium, thanks to everyone who entered, and a link. Members congratulate each other there; nothing comes back to the site. A failed post is retried on the next hourly run. Unset, nothing is posted. Treat it as a secret: anyone holding the URL can post to the channel.
